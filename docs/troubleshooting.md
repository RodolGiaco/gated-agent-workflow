...
| `the session produced no commit; nothing to push` | The session ended without committing. When the line above it reads `cost: 0 USD`, the session never reached the model, usually because the sign-in expired. Otherwise the model most likely ended its turn while a command ran in the background: a headless session ends with the turn. | Sign in again if the cost is 0. Otherwise commit or discard what the session left in the working tree, then run again. |
